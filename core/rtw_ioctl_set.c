...
extern void indicate_wx_scan_complete_event(struct adapter *adapter);
...
u8 rtw_do_join(struct adapter *adapter)
{
	...
	if (list_empty(&queue->queue)) {
		...
		if (!pmlmepriv->LinkDetectInfo.bBusyTraffic ||
			...
			ret = rtw_sitesurvey_cmd(adapter, &pmlmepriv->assoc_ssid, 1, NULL, 0);
			if (_SUCCESS != ret) {
...
handle_tkip_countermeasure:
	...
	if (adapter->securitypriv.btkip_countermeasure) {
		cur_time = jiffies;

		if ((cur_time - adapter->securitypriv.btkip_countermeasure_time) > 60 * HZ) {
			adapter->securitypriv.btkip_countermeasure = false;
			adapter->securitypriv.btkip_countermeasure_time = 0;
		} else {
			status = _FAIL;
			goto release_mlme_lock;
		}
...
u8 rtw_set_802_11_ssid(struct adapter *adapter, struct ndis_802_11_ssid *ssid)
{
	u8 status = _SUCCESS;
	u32 cur_time = 0;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE)) {
		...
		if ((pmlmepriv->assoc_ssid.SsidLength == ssid->SsidLength) &&
			...
			if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == false)) {
				...
				if (!rtw_is_same_ibss(adapter, pnetwork)) {
				...
				rtw_lps_ctrl_wk_cmd(adapter, LPS_CTRL_JOINBSS, 1);
			}
		} else {
	...
	enum ndis_802_11_network_infra networktype)
{
	...
	if (*pold_state != networktype) {
		...
		if ((*pold_state == Ndis802_11Infrastructure) || (*pold_state == Ndis802_11IBSS)) {
			if (check_fwstate(pmlmepriv, _FW_LINKED) == true)
				rtw_indicate_disconnect(adapter); /* will clr Linked_state; before this function, we must have checked whether  issue dis-assoc_cmd or not */
	       }
...
u8 rtw_set_802_11_bssid_list_scan(struct adapter *adapter, struct ndis_802_11_ssid *pssid, int ssid_max_num)
{
	struct	mlme_priv		*pmlmepriv = &adapter->mlmepriv;
	u8	res = true;
	...
	if ((check_fwstate(pmlmepriv, _FW_UNDER_SURVEY|_FW_UNDER_LINKING)) ||
		...
		res = rtw_sitesurvey_cmd(adapter, pssid, ssid_max_num, NULL, 0);
...
u8 rtw_set_802_11_authentication_mode(struct adapter *adapter, enum ndis_802_11_auth_mode authmode)
{
	...
	res = rtw_set_auth(adapter, psecuritypriv);
...
u8 rtw_set_802_11_add_wep(struct adapter *adapter, struct ndis_802_11_wep *wep)
{
	...
	res = rtw_set_key(adapter, psecuritypriv, keyid, 1);
